...
public class ProjectFactory extends WorkFactory {
    ...
    public Work create(int level) {
        ...
        return new Project(getWorkName()[ (int) Math.round(Math.random()*4)], FormulaCalculator.getInstance().calculate(hp,1.10,level,20*level)*8
                ,FormulaCalculator.getInstance().calculate(exp,1.10,level,15*level)*5,time);
